...
use lily_websocket::{
    ApplicationScopeFactory, BackgroundCancellation as ExecutionCancellation,
    BackgroundServiceTrait, Injectable, InjectionError, ProcessContext, ServiceTrait, WsAppBuilder,
};
...
impl ServiceTrait for Scoped {
    ...
    async fn dispose(&self) -> Result<(), InjectionError> {
        ...
        match std::env::var("LILY_WS_BACKGROUND_EXPORT_CASE")
            .unwrap()
            .as_str()
        {
            "cleanup_timeout" => std::future::pending().await,
        ...
        self.scopes
            ...
            .run(move |extensions| {
                Box::pin(async move {
                    ...
                    if std::env::var("LILY_WS_BACKGROUND_EXPORT_CASE").as_deref() == Ok("forced") {
...
async fn background_scope_and_dependency_events_are_flushed_with_actual_trace_identity() {
    let Ok(case) = std::env::var("LILY_WS_BACKGROUND_EXPORT_CASE") else {
        for case in ["cooperative", "forced", "cleanup_error", "cleanup_timeout"] {
            let result = bounded(tokio::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("LILY_WS_BACKGROUND_EXPORT_CASE", case)
    ...
    assert!(
        terminal[0]["fields"]["duration_ms"]
            .as_f64()
            .unwrap()
            .is_finite()
    );
    assert_eq!(app.close().await.is_err(), case.starts_with("cleanup_"));
    assert_eq!(app.health_snapshot().unwrap(), snapshot);
}
